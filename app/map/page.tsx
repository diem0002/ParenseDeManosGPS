...
import { Trophy } from 'lucide-react'; // Icon for Bets

// ... existing imports ...

// Helper to calculate percentages
const getBetStats = (fightId: string, bets: any[]) => {
    const fightBets = bets.filter(b => b.fightId === fightId);
...
// --- BETTING PERSISTENCE LOGIC ---
useEffect(() => {
    if (!group) return;

    // 1. Save to Local Backup (Hive Mind)
    if (group.bets && group.bets.length > 0) {
        localStorage.setItem(`pdm_bets_${groupCode}`, JSON.stringify(group.bets));
    }

    // 2. Recovery / Reseeding (If server is empty but we have data)
    const serverBets = group.bets || [];
    if (serverBets.length === 0) {
        const backup = localStorage.getItem(`pdm_bets_${groupCode}`);
        if (backup) {
            try {
                const localBets = JSON.parse(backup);
                // If we have local bets and server has none, we could re-seed.
                // For simplicity in this version, we just MERGE them locally for display
                // so the user sees them even if server forgot.
                // Real re-seeding would require a bulk POST API, but client-side display merge is enough for UX.
                if (localBets.length > 0) {
                    setGroup(prev => prev ? ({ ...prev, bets: localBets }) : prev);
                }
            } catch (e) { }
        }
    }
}, [group?.bets, groupCode]);
...
const handleVote = async (fightId: string, prediction: 'A' | 'B') => {
    ...
    setGroup(prev => {
        if (!prev) return null;
        // Remove my previous vote for this fight
    ...
    activeTab === 'bets' && (
        <div className="absolute inset-0 z-20 flex flex-col bg-black/95 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex-1 overflow-y-auto p-4 space-y-6 pb-24">
                ...
                {FIGHTS.map(fight => {
                    const stats = getBetStats(fight.id, group?.bets || []);
                    ...
                    return (
                        <div key={fight.id} className="bg-zinc-900 border border-white/10 rounded-xl p-4 shadow-lg">
                            ...
                            <div className="flex justify-between items-center gap-2 mb-3">
                                ...
                                <div className="text-whitefont-black text-xs opacity-50">VS</div>
...
{/* ... existing Member and Chat panels ... */ }
